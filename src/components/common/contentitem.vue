<template>
	<div>
		<div class="item">
			...
			<div class="text">
				<p><span style="font-weight: 600;cursor: pointer;" >{{obj.name}}：</span>{{display}}
				</p>
					...
					<div class="commit_2">
						<div v-for="obj,n in items.respons" :key="n">
							<div class="commit2_user">
								<span>{{obj.name1}} </span> <span style="color: lightgray;padding: 0 10px;">回复 </span><span>{{obj.name2}}
								</span> <span>{{obj.datenumber+obj.datetext}}</span>
							</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			touser() {
				this.$router.replace({
					path: '/home/user_page/first_page_1',
					query: {
						question: this.obj.question
					}
				});
